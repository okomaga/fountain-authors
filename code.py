...
def extract_authors(url):
    ...
    for div in author_divs:
        ...
        # If the text contains "Fountain", change it to "Fountain Editors"
        if "fountain" in text.lower():
            authors.append("Fountain Editors")
        else:
            authors.append(text)
    return authors
...
def main():
    ...
    # Count how many articles each author has (using Counter)
    counts = Counter(all_authors)

    # Print a table with the author and the number of articles they contributed
    print("\nAuthor vs. Number of Articles")
    print("{:<30} | {:>10}".format("Author", "Articles"))
    print("-" * 45)
    for author, count in counts.most_common():
        print("{:<30} | {:>10}".format(author, count))
